...
func Extract(reader io.Reader) (pdfPages []PdfPage, err error) {
	var tsv []PopplerTsvRow
	if tsv, err = ExtractInPopplerTsv(reader); err != nil {
		return nil, err
	}
...
func ExtractOrError(reader io.Reader) (pages []PdfPage, err error) {
	if pages, err = Extract(reader); err != nil {
		return pages, err
	}
...
func ExtractInPopplerTsv(reader io.Reader) (tsvRows []PopplerTsvRow, err error) {
	params := []string{
		"-tsv",
		"-", // Read from stdin
		"-", // Write to stdout
